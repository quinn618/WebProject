...
function generateNoteCard(doc) {
    const typeLabel = doc.type === 'paid'
        ? '<span style="color:#1D9E75;font-weight:700;">' + parseFloat(doc.price).toFixed(2) + ' DT</span>'
        ...
        '<div class="note-card" data-id="' + doc.id + '">' +
            '<div class="note-header">' +
                ...
                '<div class="note-actions">' +
                    '<button class="note-btn" onclick="editDocument(' + doc.id + ')" title="Modifier">' +
                        '<span class="material-symbols-outlined">edit</span>' +
                    '</button>' +
                    '<button class="note-btn danger" onclick="deleteDocument(' + doc.id + ')" title="Supprimer">' +
            ...
            '<div class="note-footer">' +
                '<span class="note-meta">' + doc.filiere + ' · ' + doc.matiere + '</span>' +
                typeLabel +
...
async function deleteDocument(docId) {
    ...
    try {
        ...
        allDocuments = allDocuments.filter(function(d) { return d.id !== docId; });
        renderNotes();
